...
4. **Enable real-world-style tuning validation** — particularly for sunnypilot's custom longitudinal tuning — without requiring a physical test vehicle for every iteration
...
├── sunnypilot (openpilot fork) — all openpilot processes
...
- **sunnypilot** checked out and built (tested with `custom-longitudinal-tuning` branch)
...
- Per-component start/stop for sunnypilot and the BeamNG bridge
- **Controls popover** with:
  - Cruise control buttons (MAIN, CANCEL, SET▼, RES▲) for MADS engagement
...
When openpilot is engaged, physical controller inputs (steer/throttle/brake via FIFO) take priority over openpilot's commands. Braking cancels longitudinal cruise while MADS lateral control remains active, mirroring real sunnypilot override behavior.
...
openpilot is fingerprinted as a **Honda Civic 2022** (the car simulated by openpilot's sim infrastructure). The Honda Civic uses a torque-based lateral controller (`latcontrol_torque`) whose gains (`liveTorqueParameters`) and steer ratio (`liveParameters`) are learned from real Honda Civic driving data. BeamNG's simulated vehicle has completely different steering dynamics, so the controller operates with wrong feedback gains. The car can steer, but it doesn't track lanes accurately until `liveTorqueParameters` and `liveParameters` converge — which requires sustained driving time and may never fully converge to BeamNG's dynamics.

### 2. MetaDrive Also Not Lane-Keeping
The same dynamics mismatch affects the MetaDrive bridge. Additionally, `latcontrol_torque` returns `steeringAngleDeg = 0.0` (hardcoded), so MetaDrive's bridge — which reads `actuators.steeringAngleDeg` — was receiving zero steer signal. Fixed by converting `actuators.curvature` to a steering angle via the Ackermann formula using the car's wheelbase and steer ratio from `carParams`.

### 3. Camera Calibration
`liveCalibration` is pre-seeded with `rpyCalib = [0, 0, 0]` (camera perfectly level, no pitch/yaw). This is a reasonable assumption but may not match the actual camera mounting angle in the BeamNG scene. `calibrationd` re-learns this from visual odometry over time, but starts from the pre-seeded value.
